...
from structure.helpers import calculate_team_features, calculate_player_kda, prepare_data
...
class TestFeatureEngineering(unittest.TestCase):
    ...
    def test_calculate_team_features(self):
        ...
        self.assertAlmostEqual(df_radiant["radiant_avg_hero_winrate"].iloc[0], 0.6, places=1)
        self.assertAlmostEqual(df_radiant["radiant_avg_player_winrate"].iloc[0], 0.65, places=1)
        ...
        self.assertAlmostEqual(df_dire["dire_avg_player_winrate"].iloc[0], 0.55, places=1)
    ...
    def test_calculate_player_kda(self):
        ...
        self.assertAlmostEqual(df_with_kda["radiant_player_1_kda"].iloc[0], 4.0, places=1)
        self.assertAlmostEqual(df_with_kda["radiant_player_2_kda"].iloc[0], 8.0, places=1)
        self.assertAlmostEqual(df_with_kda["radiant_player_3_kda"].iloc[0], 5.5, places=1)
        self.assertAlmostEqual(df_with_kda["radiant_player_4_kda"].iloc[0], 1.0, places=1)
        self.assertAlmostEqual(df_with_kda["radiant_player_5_kda"].iloc[0], 5.0, places=1)
    ...
    def test_prepare_data(self):
        ...
        self.assertEqual(prepared_df["radiant_win"].iloc[0], 1)  # True should be converted to 1

        # Validate the normalization of features
        scaler = MinMaxScaler()
        scaled_features = scaler.fit_transform(prepared_df.drop(columns=["radiant_win"]))
